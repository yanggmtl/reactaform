...
import FileInput, { type FileField } from '../../../package/components/fields/FileInput';
import { renderWithProvider, createMockField, baseFieldProps } from '../../test-utils';

describe('FileInput', () => {
  it('renders file input button with label', () => {
    const field = createMockField<FileField>({ label: 'Upload', accept: '*' });
  ...
  it('renders choose files button for multiple files', () => {
    const field = createMockField<FileField>({ label: 'Upload', accept: '*', multiple: true });
    renderWithProvider(<FileInput {...baseFieldProps} field={field} value={null} />);
  ...
  it('calls onChange when file is selected', async () => {
    const onChange = vi.fn();
    const field = createMockField<FileField>({ displayName: 'Upload', accept: '*' });
  ...
  it('calls onChange with array for multiple files', async () => {
    const onChange = vi.fn();
    const field = createMockField<FileField>({ displayName: 'Upload', accept: '*', multiple: true });
    const { container } = renderWithProvider(
      <FileInput {...baseFieldProps} field={field} value={null} onChange={onChange} />
    );
  ...
  it('validates required field', () => {
    const onChange = vi.fn();
    const field = createMockField<FileField>({ displayName: 'Required Upload', accept: '*', required: true });
    renderWithProvider(
      <FileInput {...baseFieldProps} field={field} value={null} onChange={onChange} />
    );
  ...
  it('sets accept attribute', () => {
    const field = createMockField<FileField>({ label: 'Upload', accept: 'image/*,.pdf' });
    const { container } = renderWithProvider(
  ...
  it('displays tooltip icon when provided', () => {
    const field = createMockField<FileField>({ displayName: 'Upload', accept: '*', tooltip: 'Select a file to upload' });
    renderWithProvider(<FileInput {...baseFieldProps} field={field} value={null} />);
  ...
  it('has accessible id matching field name', () => {
    const field = createMockField<FileField>({ displayName: 'Upload', name: 'fileUpload', accept: '*' });
    const { container } = renderWithProvider(
      <FileInput {...baseFieldProps} field={field} value={null} />
    );
