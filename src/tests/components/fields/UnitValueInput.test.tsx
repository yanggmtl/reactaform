...
import UnitValueInput, { type UnitValueField } from '../../../package/components/fields/UnitValueInput';
...
describe('UnitValueInput', () => {
  it('renders text input and unit selector with label', () => {
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Distance', dimension: 'length', defaultValue: ['10', 'm'] });
  ...
  it('displays initial value and unit', () => {
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Weight', dimension: 'weight', defaultValue: ['5', 'kg'] });
  ...
  it('calls onChange when value is changed', async () => {
    ...
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Length', dimension: 'length', defaultValue: ['10', 'm'] });
    renderWithProvider(
  ...
  it('validates required field', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Required Value', dimension: 'length', required: true, defaultValue: ['10', 'm'] });
    renderWithProvider(
      <UnitValueInput {...baseFieldProps} field={field} value={['10', 'm']} onChange={onChange} />
    );
  ...
  it('validates invalid number format', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Value', dimension: 'length', defaultValue: ['10', 'm'] });
    renderWithProvider(
      <UnitValueInput {...baseFieldProps} field={field} value={['10', 'm']} onChange={onChange} />
    );
  ...
  it('disables conversion button when value is empty', () => {
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Distance', dimension: 'length', defaultValue: ['', 'm'] });
  ...
  it('disables input when disabled prop is true', () => {
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Disabled', dimension: 'length', disabled: true, defaultValue: ['10', 'm'] });
    renderWithProvider(<UnitValueInput {...baseFieldProps} field={field} value={['10', 'm']} />);
  ...
  it('displays tooltip icon when provided', () => {
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Value', dimension: 'length', tooltip: 'Enter length value', defaultValue: ['10', 'm'] });
    renderWithProvider(<UnitValueInput {...baseFieldProps} field={field} value={['10', 'm']} />);
  ...
  it('returns null when dimension is not provided', () => {
    const field = createMockField<UnitValueField>({ type: 'unitvalue', label: 'Value', defaultValue: ['10', 'm'] });
    const { container } = renderWithProvider(
      <UnitValueInput {...baseFieldProps} field={field} value={['10', 'm']} />
    );
